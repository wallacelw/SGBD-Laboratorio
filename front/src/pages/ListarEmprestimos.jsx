...
function Emprestimos() {
  ...
  const handleEdit = async (emprestimo) => {
    navigate(
      `/Edit-Emprestimo/${emprestimo.id}/${emprestimo.id_do_livro}/${emprestimo.id_do_material}/${emprestimo.id_do_usuario}`
    );
  };
  ...
  const handleDelete = async (emprestimo) => {
    try {
      if (emprestimo.id_do_livro) {
        await axios
          .delete(
            `http://localhost:3333/emprestimo/byISBN/${emprestimo.id}/${emprestimo.id_do_livro}`,
            { headers: headers }
          )
        ...
        await axios
          .delete(
            `http://localhost:3333/emprestimo/byID/${emprestimo.id}/${emprestimo.id_do_material}`,
            { headers: headers }
          )
          .then((res) => toast(res.data.message));
      }
  ...
  useEffect(() => {
    fetch("http://localhost:3333/emprestimos")
      ...
      .then((data) => {
        ...
        if (localStorage.getItem("authLevel") < 2) {
          setEmprestimos(
            emprestimosFormatados.filter(
              (emprestimo) =>
                emprestimo.id_do_usuario == localStorage.getItem("userId")
            )
          );
          setEmprestimosFiltrados(
            emprestimosFormatados.filter(
              (emprestimo) =>
                emprestimo.id_do_usuario == localStorage.getItem("userId")
            )
        ...
        <tbody>
          {emprestimosFiltrados.map((emprestimo, index) => (
            <tr className="tabela" key={index}>
              ...
              <td>
                <button onClick={() => handleReturn(emprestimo)}>
                  Devolver
                </button>
              </td>
              {isAdmin ? (
                <>
                  <td>
                    <button onClick={() => handleEdit(emprestimo)}>
                      Editar
                    </button>
                  </td>
                  <td>
                    <button onClick={() => handleDelete(emprestimo)}>
                      Apagar
                    </button>
                  </td>
                </>
